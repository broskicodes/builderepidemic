...
export default function DashboardPage() {
  ...
  const stripeCheckout = async () => {
    const response = await fetch("/api/stripe/checkout", {
      method: "POST",
      body: JSON.stringify({ priceId: process.env.NEXT_PUBLIC_PRICE_ID_10, user: session?.user }),
    });
        ...
        {clicked ? (
          ...
          <section className="container flex flex-col items-center gap-8 pb-28 pt-20 sm:gap-10">
            ...
            <div className="grid grid-cols-2 gap-3">
              <Button size="lg" variant="outline" className="cursor-pointer border-border" onClick={stripeCheckout}>
                Learn More
              </Button>

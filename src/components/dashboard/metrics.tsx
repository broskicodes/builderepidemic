...
import { ArrowDownIcon, ArrowUpIcon, BarChart3Icon, HashIcon, Eye, ThumbsUp, Bookmark, MessageCircle, Repeat } from "lucide-react"
...
const metricToDataKey: Record<Metric, (tweet: Tweet) => number> = {
  ...
  engagement_rate: (tweet) => {
    const engagement = tweet.like_count + tweet.reply_count + tweet.bookmark_count + 
...
export const Metrics = ({ tweets, metricLabels }: MetricsProps) => {
  ...
  const stats = useMemo(() => {
    return Object.keys(metricLabels).reduce((acc, metric) => {
      const getValue = metricToDataKey[metric as Metric]
      const currentValues = tweets.map(tweet => getValue(tweet))
      const prevValues = tweets.map(tweet => getValue(tweet))
      
      if (metric === 'engagement_rate') {
        const totalEngagement = tweets.reduce((sum, tweet) => 
          sum + tweet.like_count + tweet.reply_count + tweet.bookmark_count + 
          tweet.retweet_count + tweet.quote_count, 0)
        ...
        const averageRate = tweetRates.reduce((sum, rate) => sum + rate, 0) / tweetRates.length

        const prevTotalEngagement = tweets.reduce((sum, tweet) => 
          sum + tweet.like_count + tweet.reply_count + tweet.bookmark_count + 
          tweet.retweet_count + tweet.quote_count, 0)
        const prevTotalImpressions = tweets.reduce((sum, tweet) => sum + tweet.view_count, 0)
        const prevTotalRate = prevTotalImpressions > 0 ? (prevTotalEngagement / prevTotalImpressions) * 100 : 0
        
        const prevTweetRates = tweets.map(tweet => {
          const engagement = tweet.like_count + tweet.reply_count + tweet.bookmark_count + 
                           tweet.retweet_count + tweet.quote_count
          return tweet.view_count > 0 ? (engagement / tweet.view_count) * 100 : 0
        })
        const prevAverageRate = prevTweetRates.reduce((sum, rate) => sum + rate, 0) / prevTweetRates.length

        const totalChange = prevTotalRate > 0 ? ((totalRate - prevTotalRate) / prevTotalRate) * 100 : 0
        const avgChange = prevAverageRate > 0 ? ((averageRate - prevAverageRate) / prevAverageRate) * 100 : 0
        ...
        const prevTotal = prevValues.reduce((sum, value) => sum + value, 0)
        
        const currentAvg = currentTotal / currentValues.length
        const prevAvg = prevTotal / prevValues.length
        
        const totalChange = ((currentTotal - prevTotal) / prevTotal) * 100
        const avgChange = ((currentAvg - prevAvg) / prevAvg) * 100
      ...
      return acc
    }, {} as Record<Metric, { total: number; average: number; totalChange: string; averageChange: string }>)
  }, [tweets])
      ...
      <Card className="w-full mb-6">
        ...
        <CardContent>
          <div className="flex justify-between mb-4">
            ...
            <ToggleGroup type="single" value={statType} onValueChange={handleStatTypeChange} size="sm">
              ...
              <Card key={metric} className="w-full">
                ...
                <CardContent>
                  ...
                  <div className="flex items-center pt-1">
                    {parseFloat(statType === 'total' ? stats[metric].totalChange : stats[metric].averageChange) > 0 ? (
                      <ArrowUpIcon className="h-4 w-4 text-green-500" />
                    ) : (
                      <ArrowDownIcon className="h-4 w-4 text-red-500" />
                    )}
                    <span className={`text-xs font-medium ${parseFloat(statType === 'total' ? stats[metric].totalChange : stats[metric].averageChange) > 0 ? 'text-green-500' : 'text-red-500'}`}>
                      {statType === 'total' ? stats[metric].totalChange : stats[metric].averageChange}%
                    </span>
                  </div>
                </CardContent>
              </Card>
